...
### 5.4 数据查询工具集 (Data Query Tools) [New]
基于水鱼查分器 (Diving Fish) API 构建的查询模块。

#### 5.4.1 B50 成绩查询 (Best 50)
*   **功能描述**: 展示玩家的 Rating 构成 (35首新曲 + 15首旧曲)。
*   **数据源**: `POST /query/player` (带 `b50: true` 参数) 或 `GET /player/records` (Token模式本地计算)。
*   **UI 设计**:
    *   **头部**: 玩家信息卡片 (昵称, Rating, 段位, 称号)。
    *   **图表**: Rating 构成饼图或进度条 (DX vs Standard)。
    *   **列表**: 
        *   **DX 分区 (35曲)**: 网格或列表展示，按 Rating 降序。
        *   **Standard 分区 (15曲)**: 同上。
    *   **卡片细节**: 封面、曲名、难度(颜色)、达成率(SSS+等)、定数、单曲Rating。
    *   **分享功能**: 生成 B50 长图 (类似查分器官方或第三方生成的图片)。

#### 5.4.2 查分/成绩列表 (Score List)
*   **功能描述**: 查看玩家所有游玩记录，支持多维度筛选。
*   **数据源**: `GET /player/records` (需 Token) 或 `POST /query/player` (QQ号查询可能受限，主要依赖 Token)。
*   **筛选/排序功能**:
    *   **筛选**: 等级 (1-15), 难度 (Basic-Re:Master), 版本, 达成率 (SSS+, FC/AP)。
    *   **排序**: 达成率, 定数, 游玩时间(如果API支持)。
*   **展示形式**:
    *   **列表模式**: 紧凑信息，适合快速浏览。
    *   **封面模式**: 强调视觉，类似选曲界面。

#### 5.4.3 详细成绩查询 (Detailed Score)
*   **功能描述**: 针对单曲的详细成绩展示。
*   **入口**: 点击 B50 或 成绩列表中的单曲卡片。
*   **内容**:
    *   **谱面信息**: 详细定数、BPM、谱师。
    *   **成绩详情**: 
        *   达成率 (Achievement)。
        *   评级 (Rank)。
        *   DX Score / 满分 DX Score (星星数)。
        *   Combo 状态 (FC/AP/Sync)。
        *   判定详情 (Critical/Perfect/Great/Good/Miss) - *注: 水鱼API通常不返回具体的判定数，除非是特定接口，需确认*。
    *   **历史趋势**: 如果本地有缓存历史数据，可展示该曲目的成绩变化曲线 (进阶功能)。
