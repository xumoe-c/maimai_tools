...
const fitDiffDisplay = computed(() => {
    return props.record.fit_diff ? props.record.fit_diff.toFixed(1) : props.record.ds.toFixed(1)
})

const isFit = computed(() => props.record.is_fit)
</script>
  ...
  <div class="relative bg-white border-2 border-black flex flex-col select-none h-full shadow-[4px_4px_0px_0px_#000000] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_#000000] transition-all">
    ...
    <div class="relative z-10 flex flex-col h-full">
        ...
        <div class="p-2 flex flex-col gap-1 flex-1 justify-between bg-white">
            ...
            <!-- Fit Diff Info -->
            <div class="flex items-center gap-1 text-[10px] leading-none mt-0.5">
                 <span class="font-bold text-gray-500">DS:</span>
                 <span :class="isFit ? 'text-maimai-pink font-black' : 'text-black'">{{ fitDiffDisplay }}</span>
                 <span v-if="isFit" class="text-[8px] text-gray-400 line-through">{{ record.ds.toFixed(1) }}</span>
            </div>
